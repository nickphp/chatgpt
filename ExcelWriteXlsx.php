...
/**
 * Excel Xlsx 数据导出工具类
 * 1.支持多Sheet同时输出到Excel
 * 2.支持指定单元格合并 静态指定和动态指定
 * 3.表格自动文字居中
 * 4.可根据OpenXML协议规范任意修改 实现更复杂的功能
 * 5.导出数据时，性能优越，占用极少内存，支持十万级别的数据导出无压力
 */
class ExcelWriteXlsx {
    ...
    protected function initializeSheet($sheet_name) {
        if ($this->current_sheet==$sheet_name || 
            isset($this->sheets[$sheet_name])) {
            return;
        }
	...
	protected function finalizeSheet($sheet_name)
	{
		...
		if (!empty($sheet->merge_cells)) {
			$sheet->file_writer->write(    '<mergeCells style="align:center;valign:center;">');
	...
	protected function writeStylesXML()
	{
		...
		$file->write(		'<font><name val="Arial"/><family val="0"/><sz val="10"/></font>');
		$file->write(		'<font><name val="Arial"/><family val="0"/><sz val="10"/><color rgb="FFFFFFFF"/></font>');
		$file->write('</fonts>');
		$file->write('<fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF200"/><bgColor rgb="FFFFFF00"/></patternFill></fill></fills>');
         ...
         *
         * 1. horizontal="left" vertical="top"   (左上)
         * 2. horizontal="center" vertical="top" (中上)
         * 3. horizontal="right" vertical="top"  (右上)
         * 4. horizontal="left" vertical="center" (左中)
         * 5. horizontal="center" vertical="center" (中中)
         * 6. horizontal="right" vertical="center" (右中)
         * 7. horizontal="left" vertical="bottom" (左下)
         * 8. horizontal="center" vertical="bottom" (中下)
         * 9. horizontal="right" vertical="bottom" (右下)
         *
         * 只要修改horizontal和vertical对应的参数就可以改变单元格元素的布局
         *
         * 修改默认背景色指定一个填充fillId 该id对应fills标签内的fill默认id号从0开始
         * 修改默认文字颜色指定一个fontId 该id对应fonts标签内的font默认id号从0开始
         *
         */
		$file->write(   '<cellXfs count="2">');
        $file->write(       '<xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>');
        $file->write(       '<xf numFmtId="0" fontId="3" fillId="2" borderId="0" xfId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>');
		$file->write(   '</cellXfs>');
        $file->write(   '<cellStyles count="7">');
        $file->write(       '<cellStyle builtinId="0" customBuiltin="false" name="Normal" xfId="0"/>');
        $file->write(       '<cellStyle builtinId="0" customBuiltin="false" name="style1" xfId="1"/>');
...
class XLSXWriter_BuffererWriter {
    ...
    public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
    {
            ...
            if ($this->fd===false) {
                    ExcelWriteXlsx::log("Unable to open $filename for writing.");
            }
    }
    ...
    protected function purge()
    {
        if ($this->fd) {
            if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
                    ExcelWriteXlsx::log("Error, invalid UTF8 encoding detected.");
